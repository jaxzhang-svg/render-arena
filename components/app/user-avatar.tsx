...
import { trackHackathonJoinClicked, trackLoginStarted } from '@/lib/analytics'
...
export function UserAvatar({ className }: UserAvatarProps) {
  ...
  // 加载中状态
  if (loading) {
    return <div className={`size-10 animate-pulse rounded-full bg-[#e7e6e2] ${className || ''} `} />
  }
  ...
  if (!user) {
    return (
      <Button
        onClick={() => {
          trackLoginStarted('header')
          loginWithNovita(pathname)
        }}
        variant="outline"
        className="border-[#e7e6e2] font-mono text-[14px] leading-[16px] text-[#292827] hover:border-[#23d57c] hover:text-[#23d57c]"
      >
        Login
      </Button>
    )
    ...
    <Popover.Root>
      <div className="relative" ref={setContainer}>
        <Popover.Trigger id={popoverTriggerId}>
          <div
            className={`flex size-8 cursor-pointer items-center justify-center overflow-hidden rounded-full border border-[#e7e6e2] bg-black text-white ring-2 ring-transparent transition-all hover:ring-[#23d57c]/30 ${className || ''} `}
          >
            <span className="font-mono text-[14px] font-semibold">{getInitial()}</span>
          </div>
        </Popover.Trigger>
          ...
          <Popover.Positioner sideOffset={8} align="end">
            ...
            >
              ...
              <div className="flex items-center justify-between px-3 py-2">
                <span className="truncate font-sans text-[16px] leading-[20px] font-medium">
                ...
                <button
                  ...
                  className="ml-2 flex size-4 cursor-pointer items-center justify-center text-gray-500 hover:text-black"
                  title="Copy email"
                >
                  <svg
                    width="14"
                  ...
                  >
                    <path
                      d="M9.33333 0.666667H4.66667C3.92933 0.666667 3.33333 1.26267 3.33333 2V3.33333H2C1.26267 3.33333 0.666667 3.92933 0.666667 4.66667V12C0.666667 12.7373 1.26267 13.3333 2 13.3333H9.33333C10.0707 13.3333 10.6667 12.7373 10.6667 12V10.6667H12C12.7373 10.6667 13.3333 10.0707 13.3333 9.33333V2C13.3333 1.26267 12.7373 0.666667 12 0.666667H9.33333ZM9.33333 12H2V4.66667H9.33333V12ZM12 9.33333H10.6667V4.66667C10.6667 3.92933 10.0707 3.33333 9.33333 3.33333H3.33333V2H12V9.33333Z"
                      fill="currentColor"
                    />
                  </svg>
                </button>
      ...
      <ArenaBattleModal open={isHackathonModalOpen} onOpenChange={setIsHackathonModalOpen} />
    </Popover.Root>
  )
}
